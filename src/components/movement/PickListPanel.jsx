        ...
        return (
          <div key={pl.id} className="border border-slate-200 rounded-xl overflow-hidden">
            ...
            <div className="px-4 py-3 space-y-3">
              ...
              {/* Download signed PDF */}
              {pl.status === 'Completed' && pl.signature_data && (
                <button
                  onClick={async () => {
                    setDownloadingId(pl.id);
                    try {
                      await exportPickList(movement, pl.pick_list_items || [], pl.signature_data, pl.signature_name);
                    } catch {}
                    setDownloadingId(null);
                  }}
                  disabled={downloadingId === pl.id}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold hover:bg-emerald-100 disabled:opacity-60 cursor-pointer transition-colors"
                >
                  ...
                  {downloadingId === pl.id ? 'Generating…' : 'Download Signed PDF'}
                </button>
              )}
            </div>
          </div>
